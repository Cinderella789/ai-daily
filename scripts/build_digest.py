...
def render_html(items: list[dict]) -> str:
    ...
    <html><body style="font-family:-apple-system,Segoe UI,Roboto,sans-serif;max-width:640px;margin:0 auto;padding:24px;">
      ...
      {''.join(rows)}
      <p style="font-size:12px;color:#aaa;">Полная лента: <a href="https://ai-daily.pplx.app">ai-daily.pplx.app</a></p>
    </body></html>
    """
...
def main() -> None:
    ...
    if "--dry-run" in sys.argv or not os.environ.get("SMTP_HOST"):
        ...
        return
    send(html, subject="AI Daily — топ-5 новостей за сутки")
    print(f"[build_digest] отправлено {len(top)} новостей на {os.environ['DIGEST_TO']}")
